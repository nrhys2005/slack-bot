...
- anthropic SDK (Claude API) — @멘션 질문 답변용
...
├── chat.py            # Claude API로 태스크 출력 분석 및 질문 답변
...
@bot 지금 어디까지 됐어?
  ...
  → chat.py: 누적 출력을 Claude API에 전달, 자연어 답변 생성
  → 스레드로 답변 전송
```
...
### chat.py
- `anthropic.AsyncAnthropic` 클라이언트 (ANTHROPIC_API_KEY 환경변수)
- `answer_question(question, tasks)`: 태스크 출력 최근 100줄을 컨텍스트로 Claude API 호출
